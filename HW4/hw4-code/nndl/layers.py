...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    minimean = np.mean(x,axis=0)
    minivar = np.var(x,axis=0)
    
    running_mean = momentum * running_mean + (1 - momentum) * minimean
    running_var = momentum * running_var + (1 - momentum) * minivar
    
    xNormalized = (x-minimean) / np.sqrt(minivar+eps)
    
    out = gamma * xNormalized + beta
    
    cache = (xNormalized, x, gamma, eps, minimean, minivar)
    ...
    testNormalized = (x-running_mean) / np.sqrt(running_var+eps)
    out = gamma * testNormalized + beta
...
def batchnorm_backward(dout, cache):
  ...
  xNormalized, x, gamma, eps, minimean, minivar = cache
  
  dgamma = np.sum(xNormalized * dout ,axis = 0)
  dbeta = np.sum(dout,axis=0)
  
  dL_dxNorm = dout * gamma
  dxNorm_da = 1/(np.sqrt(minivar+eps))
  dL_da = dxNorm_da * dL_dxNorm
  da_dx=1
  
  dxNorm_de = -0.5*(dxNorm_da**3)*(x-minimean)
  dL_de = dxNorm_de*dL_dxNorm
  dL_dvar = np.sum(dL_de, axis=0)
  dvar_dx = (2*(x-minimean)/M)

  dL_dmu=np.sum(-dL_da, axis=0)    
  dmu_dx =1/M
  
  dx = da_dx*dL_da + dvar_dx*dL_dvar + dmu_dx*dL_dmu
  # ================================================================ #
  # END YOUR CODE HERE
  # ================================================================ #
