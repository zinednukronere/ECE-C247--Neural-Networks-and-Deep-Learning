...
def sgd_nesterov_momentum(w, dw, config=None):
  ...
  # ================================================================ #
  vOld = v
  v = config['momentum']*vOld - config['learning_rate']*dw 
...
def rmsprop(w, dw, config=None):
  ...
  newLearnRate = config["learning_rate"]/(np.sqrt(config['a'])+config['epsilon'])
  next_w = w - newLearnRate*dw
...
def adam(w, dw, config=None):
  ...
  updateV = (1/(1-config['beta1']**config['t']))*config['v']
  updateA = (1/(1-config['beta2']**config['t']))*config['a']
  
  newLearnRate = config["learning_rate"]/(np.sqrt(updateA)+config['epsilon'])
  
  next_w = w - newLearnRate*updateV
